...
async def create_user(data: UserCreate, response: Response, session: Session = Depends(get_session)):
   try:
...
async def get_user(user_id: int, response: Response, session: Session = Depends(get_session)):
   try:
...
async def update_user(data: UserUpdate, user_id: int = Depends(authenticate_token), session: Session = Depends(get_session)):
...
async def delete_user(user_id: int = Depends(authenticate_token), session: Session = Depends(get_session)):
   curr_user = validate_user(user_id=user_id, session=session)
   try:
